...
| --- | --- |
| `{{model}}` | Эффективная предпочитаемая модель для текущего запроса. К нативной модели без селектора, указанной в `injectionModel`, по возможности добавляется селектор аккаунта; если предпочитаемую модель определить не удалось, используется настроенное значение `injectionModel`, а если оно не задано — пустая строка |
