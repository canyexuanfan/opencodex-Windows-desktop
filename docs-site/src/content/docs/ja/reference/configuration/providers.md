...
| `autoSwitchThreshold?` | `number` | `80` |新しいセッションのプール切り替えの使用率。 `0` を設定してクォータの切り替えを無効にします。 |
| `accountPoolStrategy?` | `"quota" \| "round-robin" \| "fill-first"` | `"quota"` |新しいセッションのアカウントローテーション戦略。既存のスレッド ID はアフィニティを保持します。 |
| `accountPoolStickyLimit?` | `number` | `1` |成功した新しいセッションのバインドは 1 つのラウンドロビン選択で維持されました。範囲は 1 ～ 100。 |
...
ダッシュボードで **Codex Auth** を使用して、プール アカウントを追加し、クォータを更新します。 `config.json` は非機密メタデータを保存します。アクセス トークンとリフレッシュ トークンは、強化された資格情報ストアを使用します。既存のスレッド ID はアカウント アフィニティを保持します。新しいセッションは、`accountPoolStrategy`、クォータ、クールダウン、およびヘルスによってルーティングされます。プレストリーム 429 または 402 は、同じリクエスト内の適格な代替アカウントで 1 回再試行します。

アカウントを一時停止すると、クォータ メタデータは保持されますが、切り替え、フェイルオーバー、回復プローブ、および手動アクティベーションからは除外されます。また、そのアカウントのスレッド アフィニティもクリアされます。実行中のリクエストでは、取得された資格情報が保持されます。後のターンはルートが変更されます。すべてのアカウントが一時停止されている場合、プールのルーティングはサイレントにアカウントを選択するのではなく、失敗します。 **枯渇したアカウントを一時停止**すると、対象となるアカウントが利用可能な認証情報で更新され、100% 新たに確認されたアカウントのみが一時停止されます。不明な更新または失敗した更新は変更されません。

|戦略 |行動 |
| --- | --- |
| `quota` (デフォルト) |アクティブな使用量が `autoSwitchThreshold` を超える場合は、5 時間、週、および 30 日の期間で使用量が最も低い適格なアカウントを選択します。 `0` はクォータの選択を無効にします。 |
| `round-robin` |滑らかに均一に広がります。 `accountPoolStickyLimit` は、1 つの選択に対して 1 ～ 100 の成功した新しいセッション バインドを保持します。 |
| `fill-first` |クールダウン、再認証、または設定されたしきい値までアクティブなアカウントを維持し、その後は安定した順序で進みます。不明な使用法が強制的に切り替えられることはありません。 |

ローテーションはプロバイダーの強制から保護するものではありません。マルチアカウントの使用はプロバイダーの規約に違反する可能性があります。
