...
function writeShellEnvFile(port: number, config: OcxConfig, modelEnv: Record<string, string> = {}): void {
  const lines = [
 ...
 */
async function computeEffectiveModelEnv(config: OcxConfig): Promise<Record<string, string>> {
  ...
  return effectiveModelEnv(config.claudeCode, windows ?? {});
}
...
export async function injectSystemEnv(port: number, config: OcxConfig): Promise<SystemEnvResult> {
  ...
  const modelEnv = await computeEffectiveModelEnv(config);
  for (const [name, value] of Object.entries(modelEnv)) {
  ...
  writeShellEnvFile(port, config, modelEnv);
