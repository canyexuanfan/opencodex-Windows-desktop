...
| `autoSwitchThreshold?` | `number` | `80` | Процент использования, при котором Pool переключается для новых сессий. Значение `0` отключает quota-based switching. |
| `accountPoolStrategy?` | `"quota" \| "round-robin" \| "fill-first"` | `"quota"` | Стратегия ротации аккаунтов для новых сессий; существующие thread-id сохраняют affinity. |
| `accountPoolStickyLimit?` | `number` | `1` | Сколько успешных new-session bind'ов держать на одном round-robin-выборе. Диапазон 1–100. |
...
Используйте **Codex Auth** в дашборде, чтобы добавлять аккаунты пула и обновлять quota. В
`config.json` хранятся только не-секретные метаданные; access token'ы и refresh token'ы уходят в
hardened credential store. Существующие thread-id сохраняют account affinity. Новые сессии
маршрутизируются по `accountPoolStrategy`, quota, cooldown и health. До начала потока единичный
429 или 402 может в рамках того же запроса перевести ход на другой подходящий аккаунт.

Постановка аккаунта на паузу сохраняет его quota metadata, но исключает его из switching,
failover, recovery probe и ручной активации. Также очищается thread affinity этого аккаунта.
In-flight-запросы сохраняют уже захваченные credential'ы; последующие ходы будут перераспределены.
Если на паузе оказались все аккаунты, Pool-routing завершится ошибкой, а не выберет один молча.
Функция **Pause exhausted** обновляет eligible-аккаунты с рабочими credential'ами и ставит на
паузу только те, у кого по свежим данным confirmed 100% usage; unknown или failed-refresh-состояния
не меняются.

| Стратегия | Поведение |
| --- | --- |
| `quota` (default) | Когда active usage пересекает `autoSwitchThreshold`, выбирается eligible-аккаунт с наименьшим известным usage по окнам 5 часов, неделя и 30 дней. Значение `0` отключает quota-based selection. |
| `round-robin` | Гладкое равномерное распределение. `accountPoolStickyLimit` удерживает 1–100 успешных bind'ов новых сессий на одном выборе. |
| `fill-first` | Использовать активный аккаунт до cooldown, reauthentication или заданного порога, затем переходить дальше в стабильном порядке. Unknown usage само по себе переключение не вызывает. |

Ротация не защищает от enforcement со стороны провайдера; использование нескольких аккаунтов может
нарушать его Terms.
