...
1. `bun test tests/usage-cost.test.ts` — 세 provider가 모두 non-null이며 exact 비용·상태·출처가 맞는지 확인한다.
2. `bun run typecheck` — 새 상수와 overlay 행의 타입을 확인한다.
3. `rg -n 'claude-opus-5' src/usage/expected-prices.ts tests/usage-cost.test.ts` — 세 provider 행과 테스트 열거가 모두 있는지 확인한다.
4. 가능하면 Anthropic/Cursor/Kiro 각각의 `claude-opus-5` 샘플 사용량으로 Logs `~$`가 숫자를 표시하는지 GUI smoke를 수행한다.
