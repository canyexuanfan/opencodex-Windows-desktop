...
OpenAI 身份固定为三种：bare native id 是 `openai` Direct，`openai-multi/<model>` 是包含主账户的
多账户池，`openai-apikey/<model>` 是 API key。Direct 不轮换。API GPT-5.6 使用 1,050,000
...
[Codex 集成](/opencodex/zh-cn/guides/codex-integration/)。
...
| 路由 | 选择器 id 与目录元数据 |
| --- | --- |
| Codex Direct | `gpt-5.6-sol`、`gpt-5.6-terra`、`gpt-5.6-luna`（372,000 token） |
| Codex 多账户 | `openai-multi/gpt-5.6-sol`、`terra`、`luna`（372,000） |
...
[子代理界面](/opencodex/zh-cn/guides/sub-agent-surface/)。
:::
