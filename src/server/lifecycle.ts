...
const activeTurnStartedAt = new WeakMap<AbortController, number>();
...
export function tryAdmitTurn(): ActiveTurnLease | null {
  ...
  const lease: ActiveTurnLease = {
    bindAbortController(ac) {
      ...
      activeTurnStartedAt.set(ac, Date.now());
    },
...
export function getActiveTurnMetrics(at = Date.now()): ActiveTurnMetrics {
  ...
  for (const controller of activeTurns.keys()) {
    const startedAt = activeTurnStartedAt.get(controller) ?? at;
  ...
  return { count: turnGate.metrics().active, oldestAgeMs, ageBuckets };
}
