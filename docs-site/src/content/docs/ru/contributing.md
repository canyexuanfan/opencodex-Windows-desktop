...
- `dev2-go` — параллельная линия интеграции для нативного порта на Go (`go/`, точка входа
  нативного рантайма и инструменты сборки Go-релизов). **Пока не принимает pull request'ы.**
  Автоматическая проверка целевой ветки не знает об этой линии: она добавляет к заголовку
  префикс `[WRONG BRANCH]` и переводит PR в черновик, а при правке заголовка или переводе
  в ready запускается снова, поэтому состояние черновика сохраняется. GitHub не сливает
  черновики, так что такой PR не попадёт в репозиторий. Пока проверка не обновлена,
  отправляйте работу по нативному порту в `dev` или сначала спросите мейнтейнера.
