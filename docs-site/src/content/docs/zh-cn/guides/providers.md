...
## OpenAI 的三个 provider 层级

| Provider id | 用途 | 凭证/账户规则 |
| --- | --- | --- |
| `openai` | Codex Direct | 只使用当前 Codex caller/主登录；不读取或轮换账户池。 |
| `openai-multi` | Codex 多账户 | 主账户和添加的账户都属于池；按 affinity、配额、冷却和健康状态选择。 |
| `openai-apikey` | OpenAI API | 只使用配置的 API key/key pool；不读取 Codex 账户。 |

用 `gpt-5.6-sol`、`openai-multi/gpt-5.6-sol`、`openai-apikey/gpt-5.6-sol` 分别选择
Direct、Multi 和 API。层级之间不会凭证 fallback。API 层元数据为 1,050,000 context /
922,000 max input；`*-pro` virtual id 保留在公开状态中，线上改写为 base 模型加
`reasoning.mode: "pro"`。

首次升级时，旧账户池配置迁移到 Multi，公开的 `chatgpt` id 被隐藏。原配置只保留一次到
`~/.opencodex/config.json.pre-openai-tiers-v1.bak`；恢复命令：
...
**GitLab Duo** 是多模型 gateway，映射到各自的通用 OpenAI 兼容端点；它们使用 Bearer **订阅令牌**
（而非普通 API 密钥）进行认证，并且 Copilot 可能需要通过提供商的 `headers` 设置 `User-Agent`。
