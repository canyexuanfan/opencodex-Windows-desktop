...
function fixtureAdapter(provider: OcxProviderConfig): ProviderAdapter {
  ...
  return {
    ...
    buildRequest(parsed) {
      const body = JSON.stringify({ model: parsed.modelId, messages: parsed.context.messages });
...
function request(stream: boolean, input: unknown = "please answer"): Request {
  return new Request("http://localhost/v1/responses", {
    method: "POST",
    headers: { "content-type": "application/json" },
    body: JSON.stringify({ model: "fixture/model", input, stream }),
  });
}
