...
OpenAI에서는 프로바이더 id가 권한 경계입니다. bare `gpt-*`는 `openai` Direct,
`openai-multi/<model>`은 메인+추가 계정 풀, `openai-apikey/<model>`은 API key transport를
명시적으로 선택합니다. 세 경로는 서로 fallback하지 않으며 레거시 공개 `chatgpt` id는 마이그레이션 후 숨겨집니다.
...
3. **빌트인 프리픽스 패턴** — id를 알려진 모델 제품군 프리픽스와 대조한 뒤, 해당 이름(또는 이름
   ...
   | `gpt-`, `o1-`, `o3-`, `o4-` | bare id는 `openai` Direct; 다른 티어는 `openai-multi/` 또는 `openai-apikey/`를 명시 |
