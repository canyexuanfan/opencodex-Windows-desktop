...
opencodex поставляется с 67 встроенными пресетами: 59 на основе ключей, четыре OAuth, три локальных и
...
настроенный bearer-ключ; для доступа к API требуется план Provider (подписчики Go/Pro могут использовать
CLI-мост аутентификации). Ключи создаются в [Command Code Studio](https://commandcode.ai/studio/).
