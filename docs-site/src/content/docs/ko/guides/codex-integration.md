...
OpenAI 경로는 정확히 셋입니다. bare 모델은 현재 Codex caller/메인 로그인만 쓰는 `openai` Direct,
`openai-multi/<model>`은 메인+추가 계정 풀, `openai-apikey/<model>`은 API key만 사용합니다.
경로 간 조용한 fallback은 없습니다. 레거시 풀 config는 한 번 Multi로 이관되고 공개 `chatgpt` id를
숨기며 수동 복원을 위해 `config.json.pre-openai-tiers-v1.bak`을 보존합니다.
...
- **Direct 다음 Multi forward 후보:** caller OAuth를 우선하며 Direct가 적용되지 않을 때 설정된
  Multi 풀에서 적격 메인/추가 계정을 선택할 수 있습니다.
- **OpenAI API key:** forward 후보가 인증 실패를 소유하지 않을 때만 사용합니다. 깨진 Multi 인증을
