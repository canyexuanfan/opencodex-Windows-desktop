...
export default function IntegrationsOverview({
  ...
  const statesResource = useDataSurface<IntegrationStatus[]>(
    ...
    [apiBase],
    fetchStates,
    { isEmpty: rows => rows.length === 0, enabled: active, sessionCacheKey: `ocx.integrations.states.v1:${apiBase}`, staleAfterMs: 60_000 },
  );
  const historyResource = useDataSurface<IntegrationJournalRow[]>(
    `integration-journal-all:${apiBase}`,
    [apiBase],
    fetchHistory,
    { isEmpty: rows => rows.length === 0, enabled: active, sessionCacheKey: `ocx.integrations.journal.v1:${apiBase}`, staleAfterMs: 60_000 },
  );
  const codexResource = useDataSurface(
    `integration-codex:${apiBase}`,
    [apiBase],
    fetchCodex,
    { isEmpty: value => value === null, enabled: active, sessionCacheKey: `ocx.integrations.codex.v1:${apiBase}`, staleAfterMs: 60_000 },
  );
  const keysResource = useDataSurface(
    ...
    { isEmpty: () => false, enabled: active, sessionCacheKey: `ocx.integrations.keys.v1:${apiBase}`, staleAfterMs: 60_000 },
  );
  const claudeResource = useDataSurface(
    `integration-claude:${apiBase}`,
    [apiBase],
    fetchClaude,
    { isEmpty: value => value === null, enabled: active, sessionCacheKey: `ocx.integrations.claude.v1:${apiBase}`, staleAfterMs: 60_000 },
  );
  const claudeDesktopResource = useDataSurface(
    `integration-claude-desktop:${apiBase}`,
    [apiBase],
    fetchClaudeDesktop,
    { isEmpty: value => value === null, enabled: active, sessionCacheKey: `ocx.integrations.claude-desktop.v1:${apiBase}`, staleAfterMs: 60_000 },
  );
  const grokResource = useDataSurface(
    `integration-grok:${apiBase}`,
    [apiBase],
    fetchGrok,
    { isEmpty: value => value === null, enabled: active, sessionCacheKey: `ocx.integrations.grok.v1:${apiBase}`, staleAfterMs: 60_000 },
  );
  const nativeResource = useDataSurface<NativeStatus[] | null>(
    `integration-native:${apiBase}`,
    [apiBase],
    fetchNative,
    { isEmpty: value => value === null, enabled: active, sessionCacheKey: `ocx.integrations.native.v1:${apiBase}`, staleAfterMs: 60_000 },
  );
