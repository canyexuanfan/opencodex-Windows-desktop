...
export const ru: Record<TKey, string> = {
  ...
  "dash.syncCodexSubagentDefaultsHint": "Записывает выбор выше в собственную конфигурацию Codex, чтобы новые задачи Codex начинались с этой модели. Применится при следующей синхронизации или перезапуске, а ваши собственные настройки [agents] останутся нетронутыми. По умолчанию выключено.",
  "dash.multiAgentGuidance": "Мультиагентное руководство OpenCodex",
  "dash.multiAgentGuidanceHint": "Добавляет в промпт короткую подсказку о том, как распределять работу между подагентами. Остальное не меняется: та же поверхность v1/v2, тот же список подагентов, та же маршрутизация и те же пределы effort.",
