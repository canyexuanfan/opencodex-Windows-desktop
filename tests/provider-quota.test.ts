...
describe("fetchProviderQuotaReports", () => {
  ...
  test("Command Code quota maps 5-hour/weekly windows and monthly grant share", async () => {
    await saveCredential("command-code", { access: "cc-secret", refresh: "cc-secret", expires: Date.now() + 3600_000 });
    const seen: Array<{ url: string; authorization?: string; redirect?: RequestRedirect }> = [];
    globalThis.fetch = (async (input: RequestInfo | URL, init?: RequestInit) => {
      const url = String(input);
      const headers = init?.headers as Record<string, string> | undefined;
      ...
      if (url.includes("/internal/billing/credits")) {
        return new Response(JSON.stringify({
          data: {
            monthlyCredits: 60,
            fiveHourWindow: { usedPercent: 40.5, resetsAt: "2026-08-06T18:00:00Z" },
            weeklyWindow: { usedPercent: 52 },
          },
        }), { status: 200 });
      }
      return new Response(JSON.stringify({
        data: [{ planId: "pro", status: "active", currentPeriodEnd: "2026-08-31T00:00:00Z", monthlyCreditsTotal: 100 }],
      }), { status: 200 });
    }) as typeof fetch;
    const config = {
    ...
    expect(result.reports).toHaveLength(1);
    expect(result.reports[0]?.source).toBe("commandcode:billing");
    expect(result.reports[0]?.quota).toMatchObject({
      fiveHourPercent: 40.5,
      weeklyPercent: 52,
      monthlyPercent: 40, // (100-60)/100
    });
    expect(seen).toHaveLength(2);
    expect(seen.every(row => row.authorization === "Bearer cc-secret")).toBe(true);
    expect(seen.every(row => row.redirect === "error")).toBe(true);
  });

  test("Command Code quota never sends the credential to a non-canonical base URL", async () => {
    await saveCredential("command-code", { access: "cc-secret", refresh: "cc-secret", expires: Date.now() + 3600_000 });
    const seen: string[] = [];
    globalThis.fetch = (async (input: RequestInfo | URL) => {
      seen.push(String(input));
    ...
    const config = {
      defaultProvider: "command-code",
      providers: { "command-code": { adapter: "command-code", authMode: "oauth", baseUrl: "https://attacker.example" } },
    } as OcxConfig;
  ...
  test("Command Code quota treats a 401 as terminal (drops last-good)", async () => {
    await saveCredential("command-code", { access: "cc-secret", refresh: "cc-secret", expires: Date.now() + 3600_000 });
    let rejected = false;
    globalThis.fetch = (async () => {
      if (rejected) return new Response("unauthorized", { status: 401 });
      return new Response(JSON.stringify({
        data: { monthlyCredits: 60, fiveHourWindow: { usedPercent: 40.5 }, weeklyWindow: { usedPercent: 52 } },
      }), { status: 200 });
    }) as typeof fetch;
    const config = {
      defaultProvider: "command-code",
