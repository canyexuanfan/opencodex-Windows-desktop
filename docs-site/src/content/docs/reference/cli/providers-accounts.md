...
| `context <status\|value <tokens>\|provider <name> <on\|off> [--value <tokens>]\|all <on\|off>>` | `--json` | Read or set the context-window cap, globally or per provider. `provider ... --value <tokens>` sets an explicit cap for that provider only. |
