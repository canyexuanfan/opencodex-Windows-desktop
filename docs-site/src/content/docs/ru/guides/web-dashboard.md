...
| `GET /api/providers` · `POST /api/providers` · `PATCH /api/providers?name=...` · `DELETE /api/providers?name=...` | Список, добавление/замена, включение/отключение, назначение провайдера по умолчанию (`{ "setDefault": true }` отдельно) или удаление провайдеров. Удаление текущего default переназначает на первый оставшийся включённый провайдер, если он есть. |
