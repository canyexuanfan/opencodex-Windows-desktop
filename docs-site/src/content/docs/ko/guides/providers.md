...
| --- | --- | --- |
| `openai` | Codex Direct | 현재 Codex caller/메인 로그인만 사용하며 풀을 읽거나 회전하지 않습니다. |
| `openai-multi` | Codex 멀티계정 | 메인과 추가 계정을 모두 풀에 넣고 affinity·쿼터·cooldown·health로 선택합니다. |
| `openai-apikey` | OpenAI API | 설정된 API key/key pool만 사용하며 Codex 계정을 읽지 않습니다. |

`gpt-5.6-sol`, `openai-multi/gpt-5.6-sol`, `openai-apikey/gpt-5.6-sol`로 각각 Direct,
Multi, API를 선택합니다. 티어 간 자격증명 fallback은 없습니다. API 티어는 context 1,050,000 /
max input 922,000이며 `*-pro` virtual id는 공개 상태에 유지되고 wire에서 base 모델과
`reasoning.mode: "pro"`로 바뀝니다.

첫 업그레이드에서 레거시 풀 config는 Multi로 이관되고 공개 `chatgpt` id는 숨겨집니다. 원본은
`~/.opencodex/config.json.pre-openai-tiers-v1.bak`에 한 번 보존되며 다음 명령으로 복원합니다:
