...
В Windows для создания задания Планировщика требуется повышение прав. Независимая от языка
инструкция и автоматический запрос UAC из Startup Safety доступны только для фиксированной команды
создания принадлежащего OpenCodex задания `opencodex-proxy`, когда текущий токен подтверждён как
неповышенный. Подтвердите UAC или повторите `ocx service install` в PowerShell от имени
администратора. Если состояние токена неизвестно либо сбой произошёл вне этой команды, OpenCodex
сохраняет исходную ошибку Планировщика и не запрашивает повышение.
