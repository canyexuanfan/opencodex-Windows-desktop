...
`openai`、`openai-multi`、`openai-apikey` 是固定保留 id。Direct 只使用当前 Codex caller/主登录，
不会轮换。Multi 包含主账户和添加账户，并负责 affinity、配额、冷却选择。API 只使用配置的
API key/key pool。通过 bare 模型、`openai-multi/<model>`、`openai-apikey/<model>` 选择，层级间
没有 fallback。API GPT-5.6 元数据为 1,050,000 context / 922,000 max input，Pro virtual id 在线上
改写为 base 模型加 `reasoning.mode: "pro"`。

`openaiProviderTierVersion: 1` 标记旧配置 projection 已完成。projection 前会以 no-replace 方式创建
`config.json.pre-openai-tiers-v1.bak`；有账户池的安装默认迁移到 Multi，公开 `chatgpt` id 被隐藏。
...
| `openaiProviderTierVersion?` | `1` | migration 设置 | Direct/Multi/API projection 完成标记。 |
...
`openai/gpt-5.6-sol`、`terra`、`luna`。Direct/Multi 的 Codex 目录契约为 `372000`；同步后的
