...
- **Prepare for preview-gated OpenAI rollouts.** GPT-5.6 Sol/Terra/Luna fallback entries are ready for OpenAI API key and OpenRouter routes, with `max`/`ultra` reasoning and 372k usable-context metadata when upstream access is available.
