...
라우팅된 카탈로그 항목의 GPT-5 정체성 문구도 실제 업스트림 모델 이름에 맞게 바꿉니다. 기본 선택기 레이블은
네이티브 모델 ID의 마지막 부분을 사용하므로 폭이 좁은 선택기에서도 프로바이더 네임스페이스가 모델 이름을
가리지 않습니다. 전체 `provider/model` 경로는 카탈로그 slug와 설명에 그대로 남습니다. 경로 자체를 눈에 띄게
