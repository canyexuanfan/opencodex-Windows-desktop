...
- `dev2-go` — Go 原生移植（`go/`、原生运行时入口、Go 发布产物工具链）的并行集成线。
  **目前尚不接受 pull request。** 目标分支检查的自动化还不认识这条线：它会给 PR 标题加上
  `[WRONG BRANCH]` 前缀并将其转为草稿，而且你每次修改标题或标记为 ready 时它都会再次运行，
  草稿状态因此保持不变。GitHub 不会合并草稿 PR，所以这样的 PR 无法落地。在检查更新之前，
  请把 Go 原生移植的改动提到 `dev`，或先与维护者沟通。
