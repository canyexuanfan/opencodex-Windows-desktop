...
| `context <status\|value <tokens>\|provider <name> <on\|off> [--value <tokens>]\|all <on\|off>>` | `--json` | 전역 또는 제공자별로 컨텍스트 창 한도를 읽거나 설정합니다. `provider ... --value <tokens>`는 해당 제공자에만 별도 한도를 설정합니다. |
