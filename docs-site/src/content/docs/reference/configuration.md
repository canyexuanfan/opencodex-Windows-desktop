...
| `retryOn429?` | `{ enabled?: boolean; attempts?: number; intervalMs?: number; maxIntervalMs?: number; respectRetryAfter?: boolean }` | Opt-in same-target 429 retry: wait (upstream `Retry-After` or the fixed interval) and replay the identical request on the same key before any key failover. Codex itself never retries 429, so this is the only defense for single-key providers. Defaults: `enabled: true`, `attempts: 3`, `intervalMs: 5000`, `maxIntervalMs: 60000`, `respectRetryAfter: true`. |
