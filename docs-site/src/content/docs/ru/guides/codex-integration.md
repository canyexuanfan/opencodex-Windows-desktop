...
Строка идентичности `base_instructions` у клонированного каталога переписывается на настоящее имя
upstream-модели как статические метаданные каталога. Идентичность запроса во время выполнения
обрабатывается отдельно: маршрутизирующие адаптеры заменяют живую строку идентичности GPT-5 в
Codex на model-agnostic introduction coding-agent'а.
Если имя уникально, его label в picker по умолчанию — это последний сегмент id нативной модели,
чтобы пространство имён провайдера не скрывало собственно имя модели в узких picker'ах. При
совпадении базовых имён сохраняется достаточно контекста нативного маршрута, чтобы строки можно
было различить; если один и тот же native id приходит от нескольких провайдеров, показывается и
провайдер. Полный slug каталога остаётся в описании. Если визуально важен именно маршрут,
настройте custom display name (например, `Claude Opus 5 (TeamClaude)`); custom-имена сохраняют
приоритет. Управление reasoning формируется из metadata provider/model по лестнице Codex
`low | medium | high | xhigh | max | ultra`; неподдерживаемые значения сопоставляются или
ограничиваются ещё до upstream-запроса.
