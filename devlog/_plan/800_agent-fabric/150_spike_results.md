...
- **Command:** `codex app-server generate-ts --out <tmp>` (codex-cli 0.133.0). Evidence: `spikes/spike-a/generate-ts.log`, `spikes/spike-a/schema_summary.md`, and the full generated TS schema (version-pinned to this Codex).
...
- **Conclusion:** Spike A **PASS**. Bonus design finding: native `thread/rollback` can back the handoff rollback boundary (`080` §2).
...
- **Conclusion:** Spike C **PASS**. Kernel invariants are feasible and storage-independent; production uses SQLite (§9.2). This also informs `120`: Go is a viable Supervisor language mechanically, but does **not** establish a repo Go line.
