...
이동하지 않습니다. 이 제약은 `src/codex/catalog.ts:881-884`에 직접 기록되어 있습니다.
...
| 카탈로그 항목 | Priority | 근거 |
| --- | ---: | --- |
| `subagentModels[i]` | `i` (`0`부터 `4`) | `src/codex/catalog.ts:885-896`의 featured rank map |
| 그 밖의 라우팅 모델 | `5` | `src/codex/catalog.ts:892-896`의 라우팅 항목 생성 |
| 기본 네이티브 GPT slug | `9` | `src/codex/catalog.ts:887-890`의 네이티브 항목 생성 |
| featured 목록이 있을 때 선택되지 않은 네이티브 모델 | 최소 `featured.length + 100` | `src/codex/catalog.ts:1348-1355`의 네이티브 카탈로그 병합 |

관리 API는 `src/server/management-api.ts:626-634`의 `slice(0, 5)`로 `subagentModels`를 최대
...
(`src/codex/catalog.ts:1307-1321`). 항목 생성 시 featured rank도 priority `0`부터 `4`로 변환되므로
...
수집된 목록을 필터링합니다(`src/codex/catalog.ts:1216-1237`).
