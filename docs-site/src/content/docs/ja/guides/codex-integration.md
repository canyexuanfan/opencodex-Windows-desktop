...
ルーティングされたカタログ項目の GPT-5 アイデンティティ文言も実際の上流モデル名に合わせます。
デフォルトのピッカーラベルにはネイティブモデル ID の最後のセグメントが使われるため、幅の狭いピッカーでも
プロバイダーの名前空間がモデル名を隠しません。完全な `provider/model` 経路はカタログのスラッグと説明に残ります。
