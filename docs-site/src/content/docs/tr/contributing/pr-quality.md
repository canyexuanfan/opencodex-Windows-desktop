...
- **Çapraz Platform CI.** Test paketi ve macOS işleri yalnızca CI yol filtresi (`paths:`) eşleştiğinde çalışır; Windows test matrisleri dağıtım sınırında — `main` veya `preview`'e yükseltme sırasında ve `workflow_dispatch` ile — çalışır; Windows seçili duman testi (smoke-test) matrislerine dahil kalır. Yalnızca dokümantasyon veya `devlog/` değiştiren PR'lar test kuyruğuna alınmaz.
