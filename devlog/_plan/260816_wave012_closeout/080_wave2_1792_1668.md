...
Canonicalize at the write boundary:

```diff
   const httpVersionError = upstreamHttpVersionConfigError(prov.upstreamHttpVersion);
   if (httpVersionError) return jsonError(httpVersionError, 400);
+  if (prov.upstreamHttpVersion === null || prov.upstreamHttpVersion === "") {
+    delete prov.upstreamHttpVersion;
+  }
```
...
Disposition: review the `auth-cors.ts` hunk explicitly as a security-boundary change, then apply `maintainer-sponsored` and remove `intake: hygiene-blocked`. If the hunk is not load-bearing for the HTTP-version fix, prefer dropping it and letting the PR leave the restricted surface entirely — that is the cheaper and safer path.
