...
сервиса и состояние shim, а также эффективный Codex home со скрытым именем пользователя. При
подтверждённом расхождении Windows Orca/App home добавляется инструкция по исправлению;
`CODEX_HOME` автоматически не меняется.
