...
## 3. Crash recovery (?15 crash matrix, `130`)
...
## 4. Local IPC and authentication (?6.2, ?18)
...
**Invariant:** permissions never widen without an approval event (`050` cross-machine invariants). A handoff with a wider effective policy than the source is **Tier-3 blocked** (`080` ?4).

## 6. Privacy defaults (?9.4)
