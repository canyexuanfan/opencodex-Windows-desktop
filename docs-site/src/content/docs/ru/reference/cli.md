...
В Windows для создания задания Планировщика требуется повышение прав. Действие Startup Safety в
панели может автоматически запросить UAC. При прямом запуске `ocx service install` из обычной
оболочки OpenCodex выводит инструкцию, не зависящую от языка системы: подтвердите UAC в панели или
повторите команду в PowerShell от имени администратора. Автоматическое повышение применяется только
к фиксированной команде создания принадлежащего OpenCodex задания `opencodex-proxy`.
