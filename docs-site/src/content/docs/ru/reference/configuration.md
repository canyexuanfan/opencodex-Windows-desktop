...
**Стратегии назначения и проактивного переключения:** `quota` назначает непривязанному запросу
аккаунт с наименьшим usage и может перепривязать следующий запрос существующей задачи после
`autoSwitchThreshold`. `round-robin` равномерно распределяет непривязанные запросы, а порог не
