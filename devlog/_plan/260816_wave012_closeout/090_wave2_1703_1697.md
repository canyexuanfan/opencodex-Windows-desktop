...
Nothing in the model slug distinguishes an Auto Mode side request from a user-selected bare Opus request, and the issue explicitly asks for that separation. Define a concrete predicate over the request itself — `isClaudeClassifierRequest(body)` — before any affinity is consumed:

- Auto Mode classifier turns carry a distinctive small system prompt and a single short user turn with no tool definitions and a low `max_tokens`;
- ordinary turns carry the Claude Code system prompt, tool definitions, or prior assistant/tool history.

Implement the predicate as an explicit conjunction of stable, observable body fields (no tools declared, no assistant/tool history, `max_tokens` under the configured classifier ceiling, and the classifier system-prompt marker), record which conditions matched in the route decision, and default to `false` when the evidence is ambiguous. Failing closed here means an ambiguous request is routed as an ordinary request, which is the safe direction: it can never silently borrow another session's provider.
...
`classifierModel` and `classifierFallbacks` are operator-facing configuration, so they need management-API validation, an entry in `docs-site/`, and a visible route-decision reason in diagnostics. Shipping the routing contract without the config surface would leave the fallback list unusable in practice.
