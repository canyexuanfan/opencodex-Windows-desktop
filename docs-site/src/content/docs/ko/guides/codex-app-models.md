...
OpenAI id는 세 가지로 고정됩니다. bare native id는 `openai` Direct,
`openai-multi/<model>`은 메인을 포함한 멀티계정 풀, `openai-apikey/<model>`은 API key입니다.
Direct는 회전하지 않습니다. API GPT-5.6은 context 1,050,000 / max input 922,000이고,
...
| --- | --- |
| Codex Direct | `gpt-5.6-sol`, `gpt-5.6-terra`, `gpt-5.6-luna`(372,000 토큰) |
| Codex 멀티계정 | `openai-multi/gpt-5.6-sol`, `terra`, `luna`(372,000) |
