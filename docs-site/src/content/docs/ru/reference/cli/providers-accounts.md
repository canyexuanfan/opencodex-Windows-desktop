...
| `context <status\|value <tokens>\|provider <name> <on\|off> [--value <tokens>]\|all <on\|off>>` | `--json` | Прочитать или задать context-window cap глобально либо по провайдерам. `provider ... --value <tokens>` задаёт отдельный cap только для этого провайдера. |
