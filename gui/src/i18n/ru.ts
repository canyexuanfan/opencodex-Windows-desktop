...
export const ru: Record<TKey, string> = {
  ...
  "models.contextHint": "Задаёт реальное окно Codex для этого провайдера. Если вышестоящий API не отдаёт context_window / context_length, используется это значение; большее заявленное окно только ограничивается. Оставьте поле пустым для автоматического определения.",
  ...
  "models.setAllHint": "Применяет лимит контекста {value} ко всем маршрутизируемым провайдерам. Если релей не сообщает окно, это значение становится реальным окном Codex. Нативные провайдеры не затрагиваются.",
