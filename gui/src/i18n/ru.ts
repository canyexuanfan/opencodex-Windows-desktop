...
export const ru: Record<TKey, string> = {
  ...
  "codexAuth.switchDesc": "Применяется сразу. Уже выполняющиеся запросы сохраняют прежний аккаунт, остальные переходят на этот; при этом аккаунты с тем же порядком выбора продолжают чередоваться.",
  ...
  "codexAuth.switchBackDesc": "Применяется сразу. Уже выполняющиеся запросы сохраняют прежний аккаунт, остальные переходят на аккаунт входа через приложение; при этом аккаунты с тем же порядком выбора продолжают чередоваться.",
