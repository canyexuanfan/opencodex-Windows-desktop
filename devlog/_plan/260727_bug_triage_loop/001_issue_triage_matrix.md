...
| # | 제목 요약 | 라벨 | 판정 | 근거 |
|---|-----------|------|------|------|
| 511 | Grok Build 200k — orphan 엔트리 미채택 | bug | **CLOSE-READY** | `5ff20dc0`(pre-fence 채택) + `7ba0fec3`(하위 테이블 sweep)이 `dev`에 존재. `src/grok/inject.ts`에 `findOpencodexOrphans`/`removeOrphanTables`/adopted-default 재지정 구현 확인 |
...
| 241 | 라우팅 모델이 Desktop 모델 피커에 미표시 | bug, upstream-tracking | **UPSTREAM** | 단, #539 수정이 이 증상의 일부 원인일 가능성 있음 — WP1 후 재확인 |
...
1. **#511** — CLOSE-READY. WP2에서 근거 코멘트 후 종결.
...
다만 #241은 예외적으로 WP1 후 재확인 대상이다 — Desktop 모델 피커에 라우팅 모델이 안
보인다는 증상은 #539의 경로 문제와 표면적으로 겹친다.
