...
The preview channel seeds GPT-5.6 Sol/Terra/Luna for eligible ChatGPT passthrough, OpenAI API-key,
and OpenRouter routes. Those entries advertise `max`/`ultra` reasoning and 372,000 usable-token context
metadata, but upstream account access still controls whether a request can run.
:::
