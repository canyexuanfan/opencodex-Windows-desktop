...
`src/codex/catalog.ts:881-884` 中。

因此，opencodex 通过分配更低的 priority 控制置顶位置，而不依赖数组位置。相关 priority 如下：

| 目录条目 | Priority | 来源 |
| --- | ---: | --- |
| `subagentModels[i]` | `i`（`0` 至 `4`） | `src/codex/catalog.ts:885-896` 中的 featured rank map |
| 其他路由模型 | `5` | `src/codex/catalog.ts:892-896` 中创建路由条目的逻辑 |
| 默认原生 GPT slug | `9` | `src/codex/catalog.ts:887-890` 中创建原生条目的逻辑 |
| 存在 featured 列表时未选中的原生模型 | 至少为 `featured.length + 100` | `src/codex/catalog.ts:1348-1355` 中合并原生目录的逻辑 |

管理 API 在 `src/server/management-api.ts:626-634` 中使用 `slice(0, 5)`，把
...
（`src/codex/catalog.ts:1307-1321`）。创建条目时，featured rank 还会转换为 `0` 至 `4` 的
...
已收集的列表（`src/codex/catalog.ts:1216-1237`）。
