...
| 1127 | 0xWinner98 | FIXABLE | `selectEagerPath` rejects every rewrite on Darwin (`src/lib/bun-stream-caps.ts:99`) while inline payload rewrite and budget are Win32-only (`src/server/responses/core.ts:2128`) | 070 |
