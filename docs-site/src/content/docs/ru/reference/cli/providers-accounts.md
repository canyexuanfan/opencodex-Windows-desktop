...
запрос. Потоки, уже привязанные к аккаунту, обычно сохраняют его до исчерпания, но ошибка повторной аутентификации, cooldown по квоте или серия временных сбоев снимают привязку раньше. Любая принятая запись также снимает ручное закрепление "использовать этот аккаунт сейчас" с того аккаунта, на котором оно стояло, в том числе запись того же порядка, который уже был установлен, -- это единственный способ снять закрепление, сохранив выбранный аккаунт. (Сброс активного аккаунта через management API тоже снимает закрепление, но вместе с самим выбором.) Недоступный прокси, неизвестный id аккаунта или значение вне допустимого набора завершаются
