...
```diff
   ...
   if (providerError) return jsonError(providerError, 400);
+  if (
+    (body.provider as Record<string, unknown>).upstreamHttpVersion === null ||
+    (body.provider as Record<string, unknown>).upstreamHttpVersion === ""
+  ) {
+    delete (body.provider as Record<string, unknown>).upstreamHttpVersion;
+  }
```

Note that the validator already rejects `""` (only `undefined`/`null` short-circuit at `src/config.ts:917`), so the empty-string arm is defensive only; `null` is the case that actually reaches persistence. Apply the same canonicalization on the other write paths that call `providerManagementConfigError` (`:405`, `:617`, `:646`) so replay and sync cannot reintroduce the null.
