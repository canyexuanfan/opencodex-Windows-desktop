...
OpenCodex が所有する固定の `opencodex-proxy` タスク作成コマンドが status 1 で失敗し、現在の
トークンが未昇格と確認できたときだけ、言語に依存しないフォールバックが働き、ダッシュボードの
Startup Safety アクションが UAC を自動的に要求できます。フォールバックでトークン状態を確認
できない場合は、元のスケジューラエラーを保持します。他のタスクや操作は自動昇格 marker を生成
できません。ダッシュボードの UAC を承認するか、管理者 PowerShell で `ocx service install` を
再実行してください。
