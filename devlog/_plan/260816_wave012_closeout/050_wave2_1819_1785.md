...
+  const salvaged = salvageConfigCandidate(merged, retryResult.error, raw);
+  if (salvaged) {
+    warnDroppedConfigSections(configPath, salvaged.dropped, salvaged.errors);
+    return validFileConfigDiagnostics(
+      normalizeApiKeyIds(salvaged.parsed),   // from the merged candidate
+      salvaged.rawCandidate,                 // raw doc minus the same dropped entries
+    );
+  }
