...
`ProviderAuthKind`는 `forward | oauth | key | local` 네 가지다 (`src/providers/registry.ts:9`). 2026-07-17 registry 분포는 forward 2, OAuth 6, key 42, local 3이다.

| 종류 | 예 | 실제 경로 |
|---|---|---|
| forward | `openai`, `openai-multi` | Direct는 caller/main 전달만, Multi는 메인을 포함한 계정 풀 선택. 일반 provider API key가 아니다. |
...
- bare GPT id는 `openai` Direct이며 account pool을 읽거나 회전하지 않는다.
- `openai-multi/<model>`만 메인+추가 계정의 affinity/quota/cooldown 라우팅을 활성화한다.
- `openai-apikey/<model>`은 API key/key pool만 사용한다.
- 세 tier는 서로 credential fallback하지 않는다. `chatgpt`는 migration 후 숨겨진 legacy id다.
