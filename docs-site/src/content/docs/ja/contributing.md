...
- `dev2-go` — Go ネイティブポート（`go/`、ネイティブランタイムのエントリポイント、Go
  リリースアセットのツール）向けの並行統合ラインです。**まだ PR を受け付けていません。**
  ターゲットブランチ検査の自動化がこのラインを認識しないため、PR のタイトルに
  `[WRONG BRANCH]` を付けて draft に戻します。タイトルを直しても ready にしても自動化が
  再実行され、draft のままになります。GitHub は draft の PR をマージしないため、その PR は
  取り込めません。検査が更新されるまで、Go ネイティブポートの作業は `dev` に出すか、
  先にメンテナーへ相談してください。
