...
test("providers without a live bar render their documented rate limits", async () => {
  // The shell's workspace is built from the providers map; a registry-backed
  // provider with documented limits (and no live quota report) shows in the
  // "Documented" section instead of "No rate-limit data yet".
  quotaPayload = { reports: [] };
  const withLimits = {
    openai: { adapter: "openai-responses", authMode: "forward", baseUrl: "https://chatgpt.com/backend-api/codex" },
    groq: {
      ...
      hasApiKey: true,
      rateLimits: { rpm: 30, tpm: 6000, rpd: 1000, source: "https://console.groq.com/docs/rate-limits", updatedAt: "2026-08-06" },
    },
  } as never;
  ...
  expect(text).toContain("30 req/min");
  expect(text).toContain("6K tok/min");
  expect(text).not.toContain("No rate-limit data yet");
});
